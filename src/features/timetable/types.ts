...
export type TimetableSubject = {
	...
	teacher?:
		| {
				displayName?: string;
				named?: { lastName: string };
				unknown?: { rawNotes: string };
		  }
		| string
		| null;
	classroom?:
		| string
		| {
				room?: { building: string; floor?: string | null; number: number };
				unknown?: { rawLocation: string };
		  }
		| null;
};
...
export type SchoolCalendar = {
	termRanges: {
		label: string;
		start: SchoolCalendarDate;
		end: SchoolCalendarDate;
	}[];
...
export type FriendDetail = {
	...
	locationNotificationPreferences: Array<
		"withinTenMinutes" | "withinFiveMinutes" | "arrived"
	>;
};
