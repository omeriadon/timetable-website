...
export default function FriendRequestsSheet() {
	...
	const accept = async (relationshipID: string) => {
		try {
			await apiRequest(`v1/friends/requests/${relationshipID}/accept`, { method: "POST" });
			setIncoming((current) => current.filter((friend) => friend.relationshipID !== relationshipID));
		} catch (error) {
	...
	const remove = async (relationshipID: string, incomingRequest: boolean) => {
		try {
			await apiRequest(`v1/friends/requests/${relationshipID}`, { method: "DELETE" });
			if (incomingRequest) {
				setIncoming((current) => current.filter((friend) => friend.relationshipID !== relationshipID));
			} else {
				setOutgoing((current) => current.filter((friend) => friend.relationshipID !== relationshipID));
			}
		} catch (error) {
			setStatus((error as Error).message);
		}
	...
	return (
		<div className={styles.detailSheet}>
			...
			<section className={styles.detailCard}>
				<h3>Incoming</h3>
				{incoming.length ? incoming.map((friend) => (
					<div key={friend.relationshipID} className={styles.searchResult}>
						<ProfilePicture profile={friend.friend} size={40} />
						<div><strong>{friend.friend.displayName}</strong><span>{friend.friend.email}</span></div>
						<button type="button" onClick={() => void accept(friend.relationshipID)}>Accept</button>
						<button type="button" onClick={() => void remove(friend.relationshipID, true)}>Decline</button>
					</div>
				)) : <p className={styles.detailMuted}>No incoming requests.</p>}
			</section>
			<section className={styles.detailCard}>
				<h3>Outgoing</h3>
				{outgoing.length ? outgoing.map((friend) => (
					<div key={friend.relationshipID} className={styles.searchResult}>
						<ProfilePicture profile={friend.friend} size={40} />
						<div><strong>{friend.friend.displayName}</strong><span>{friend.friend.email}</span></div>
						<button type="button" onClick={() => void remove(friend.relationshipID, false)}>Cancel</button>
					</div>
