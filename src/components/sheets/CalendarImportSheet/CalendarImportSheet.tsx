...
export default function CalendarImportSheet({ timetable, onImported }: CalendarImportSheetProps) {
	...
	const readFile = async (file: File) => {
		...
		setEvents(parsed);
		if (!parsed.length) {
			setError("No Compass-style class events were found in that calendar file.");
		}
	};
		...
		<div className={styles.detailSheet}>
			...
			<section className={styles.detailCard}>
				<div className={styles.importInstructions}>
					<p>Export the subscribed Compass calendar from Apple Calendar or another calendar app, then choose it here. Class times are matched to the six school periods from the SwiftUI client.</p>
					<label className={styles.filePicker}>
						<SymbolIcon name="doc" fallback="＋" />
						<span>{fileName ?? "Choose calendar file"}</span>
						<input type="file" accept=".ics,text/calendar" onChange={(event) => { const file = event.target.files?.[0]; if (file) void readFile(file); }} />
					</label>
					{events.length ? <p className={styles.detailMuted}>{events.length} class events ready to import.</p> : null}
				</div>
			</section>
		...
		if (line === "END:VEVENT") {
			if (current) {
				...
				if (date && title && date.day >= 0 && date.day < 5) {
					parsed.push({ title, day: date.day, startMinutes: date.startMinutes, location: current.LOCATION ? decodeText(current.LOCATION) : null });
				}
			}
			current = null;
			continue;
...
function parseDate(value: string) {
	...
	return { day: weekday - 1, startMinutes: date.getHours() * 60 + date.getMinutes() };
}
...
function buildSubjects(events: ParsedEvent[], existing: TimetableSubject[]) {
	const subjects = new Map(existing.map((subject) => [subject.id.toLowerCase(), { ...subject, slots: [] as TimetableSlot[] }]));
	for (const event of events) {
		const session = periods.reduce((closest, period) => Math.abs(period.minutes - event.startMinutes) < Math.abs(closest.minutes - event.startMinutes) ? period : closest).session;
		...
		if (!subject.slots.some((slot) => slot.day === event.day && slot.session === session)) {
