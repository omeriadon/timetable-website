...
export default function MessageSheet({ title, message, tone = "error" }: MessageSheetProps) {
	const { closeSheet } = useSheet();

	return (
		<div className={styles.detailSheet}>
			<header className={styles.detailHeader}>
				<SymbolIcon name={tone === "error" ? "exclamationmark.triangle" : "info.circle"} fallback={tone === "error" ? "!" : "i"} />
				<div>
					<h2>{title}</h2>
					<p>{message}</p>
				</div>
