...
export default function TimetableEditorSheet({ timetable, onSaved }: { timetable: OwnerTimetable; onSaved: (timetable: OwnerTimetable) => void }) {
	const { closeSheet } = useSheet();
	const [subjects, setSubjects] = useState<TimetableSubject[]>(timetable.subjects);
	...
	const toggleSlot = (subjectID: string, slot: TimetableSlot) => {
		...
		setSubjects((current) => current.map((subject) => {
			if (subject.id === subjectID) {
				const hasSlot = subject.slots.some((candidate) => candidate.day === slot.day && candidate.session === slot.session);
				return { ...subject, slots: hasSlot ? subject.slots.filter((candidate) => !(candidate.day === slot.day && candidate.session === slot.session)) : [...subject.slots, slot] };
			}
			return subject.slots.some((candidate) => `${candidate.day}:${candidate.session}` === key) ? { ...subject, slots: subject.slots.filter((candidate) => `${candidate.day}:${candidate.session}` !== key) } : subject;
		}));
	};
	...
	const save = async () => {
		...
		setError(null);
		try {
			const updated = await apiRequest<OwnerTimetable>("v1/timetables/owner", { method: "PUT", body: JSON.stringify({ subjects, expectedRevision: timetable.revision, isSearchable }) });
			onSaved(updated);
			closeSheet();
		} catch (requestError) {
	...
	return (
		<div className={styles.detailSheet}>
			<header className={styles.detailHeader}><div><h2>Edit Timetable</h2><p>Subjects and weekly periods</p></div></header>
			<section className={styles.detailCard}>
				...
				<button type="button" className={styles.rowButton} onClick={addSubject}><div className={styles.row}><span className={styles.symbol}>＋</span><span className={styles.label}>Add Subject</span></div></button>
			</section>
			<section className={styles.detailCard}>
