...
export default function FriendSearchSheet() {
	...
	useEffect(() => {
		...
		const timer = window.setTimeout(() => {
			apiRequest<FriendSearchResult[]>(`v1/friends/search?q=${encodeURIComponent(query.trim())}`)
				.then(setResults)
	...
	const requestFriend = async (userID: string) => {
		...
		try {
			...
			setResults((current) => current.map((result) => result.profile.userID === userID ? { ...result, relationship: "pendingOutgoing" } : result));
		} catch (error) {
			...
			<section className={styles.detailCard}>
				{results.length ? results.map((result) => (
					<div key={result.profile.userID} className={styles.searchResult}>
						...
						<button type="button" onClick={() => requestFriend(result.profile.userID)} disabled={result.relationship !== null}>
							{result.relationship === "friends" ? "Friends" : result.relationship ? "Requested" : "Add"}
						</button>
					</div>
				)) : <p className={styles.detailMuted}>Enter at least two characters to search.</p>}
			</section>
