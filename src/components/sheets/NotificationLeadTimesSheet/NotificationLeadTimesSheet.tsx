...
export default function NotificationLeadTimesSheet({
	...
	return (
		<div className={styles.detailSheet}>
			<header>
				<h2>{title}</h2>
				{description ? <p className={styles.detailMuted}>{description}</p> : null}
			</header>
				...
				{leadTimes.map((value) => {
					...
					return (
						<button
							key={value}
							type="button"
							className={selected ? styles.leadTimeOptionActive : styles.leadTimeOption}
							onClick={() => toggle(value)}
							aria-pressed={selected}
						>
							<span>{value} {value === 1 ? "minute" : "minutes"} early</span>
