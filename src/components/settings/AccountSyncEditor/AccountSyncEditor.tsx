...
export default function AccountSyncEditor({ initial, onSignOut }: { initial: Settings; onSignOut: () => void }) {
		...
		<>
			<section className={styles.card}>
				...
				<div className={styles.row}>
					...
					<select className={styles.inlineSelect} value={draft.calendarEventAutoDeleteDays} disabled={saving} onChange={(event) => void save({ calendarEventAutoDeleteDays: Number(event.target.value) })}>
				...
				<button type="button" className={styles.rowButton} onClick={async () => { await apiRequest("auth/logout", { method: "DELETE" }); onSignOut(); }}>
