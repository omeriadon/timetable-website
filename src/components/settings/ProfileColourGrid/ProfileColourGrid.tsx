...
export function ProfileColourGrid({ selection, onChange }: ProfileColourGridProps) {
	const palette = makePalette(6, columnCount, true, 0.92, 0.08);

	return (
		<div className={styles.grid} style={{ "--profile-grid-columns": columnCount } as CSSProperties}>
						...
						onClick={() => {
							if (selected) {
								if (selection.length > 1) {
									onChange(selection.filter((item) => !sameColour(item, colour)));
								}
								return;
							}
...
export function ProfileForegroundColourGrid({ selection, onChange }: ForegroundColourGridProps) {
...
function makePalette(
	...
	for (let row = 0; row < rowCount; row += 1) {
		...
		const lightness = topLightness - progress * (topLightness - bottomLightness);
...
function hslToColour(hue: number, saturation: number, lightness: number): Colour {
	...
	return { r: channel(hue + 1 / 3), g: channel(hue), b: channel(hue - 1 / 3), a: 1 };
}

function sameColour(left: Colour, right: Colour) {
	return Math.abs(left.r - right.r) < 0.002 && Math.abs(left.g - right.g) < 0.002 && Math.abs(left.b - right.b) < 0.002;
}
