...
export default function ArchivedEventsEditor() {
	...
	const archived = useMemo(() => {
		...
		return [...(events?.globalEvents ?? []), ...(events?.privateEvents ?? [])]
			...
			.sort((left, right) => eventDate(right).getTime() - eventDate(left).getTime());
	}, [events]);
	...
	const update = (event: CalendarEvent | null, removedID?: string) => {
		setEvents((current) => {
			...
			return {
				...current,
				globalEvents: event
					? current.globalEvents.map((item) => item.id === event.id ? event : item)
					: current.globalEvents.filter((item) => item.id !== removedID),
				privateEvents: event
					? current.privateEvents.map((item) => item.id === event.id ? event : item)
					...
					>
						<div className={styles.row}>
							...
							<span>
								...
								<small className={styles.rowMeta}>{eventDate(event).toLocaleDateString("en-AU", { dateStyle: "long" })}</small>
							</span>
