...
export default function AdminUserReportsEditor() {
	...
	const resolve = async (report: UserReport, action: "noAction" | "accountDeleted") => {
		...
		} catch (requestError) { setError((requestError as Error).message); if (action === "accountDeleted") throw requestError; } finally { setBusy(null); }
	};
	const resolveReport = (report: UserReport, action: "noAction" | "accountDeleted") => {
