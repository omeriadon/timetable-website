...
export default function AdminCalendarEditor({ kind, title }: { kind: string; title: string }) {
	const { openSheet } = useSheet();
	const [entries, setEntries] = useState<AdminCalendarEntry[] | null>(null);
	const [error, setError] = useState<string | null>(null);
	const load = () => apiRequest<AdminCalendarEntry[]>("v1/administration/calendar").then((values) => setEntries(values.filter((entry) => entry.kind === kind))).catch((requestError: Error) => setError(requestError.message));
...
function formatDate(date: { year: number; month: number; day: number }) {
	return new Date(date.year, date.month - 1, date.day).toLocaleDateString("en-AU", { day: "numeric", month: "short", year: "numeric" });
}
