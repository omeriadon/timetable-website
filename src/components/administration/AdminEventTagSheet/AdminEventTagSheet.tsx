...
export default function AdminEventTagSheet({ tag, section, onSaved }: { tag: AdminEventTag | null; section: AdminEventTagSection; onSaved: () => void }) {
	...
	const [associatedNames, setAssociatedNames] = useState(tag?.associatedNames.join("\n") ?? "");
	...
	const save = async () => {
		...
		setError(null);
		try {
			await apiRequest(`v1/administration/event-tags${tag ? `/${tag.id}` : ""}`, { method: tag ? "PUT" : "POST", body: JSON.stringify({ sectionID: section.id, slug: slug.trim(), displayName: displayName.trim(), symbol: symbol.trim() || null, colorHex, sortOrder: tag?.sortOrder ?? section.tags.length, isArchived, associatedNames: associatedNames.split(/\r?\n/).map((value) => value.trim()).filter(Boolean) }) });
			onSaved();
			closeSheet();
		} catch (requestError) {
	...
	const remove = async () => {
		...
		setSaving(true);
		try {
			await apiRequest(`v1/administration/event-tags/${tag.id}`, { method: "DELETE" });
			onSaved();
			closeSheet();
		} catch (requestError) {
			setError((requestError as Error).message);
	...
	return (
		<div className={styles.detailSheet}>
			<header className={styles.detailHeader}><div><h2>{tag ? "Edit Tag" : "Add Tag"}</h2><p>{section.displayName}</p></div></header>
