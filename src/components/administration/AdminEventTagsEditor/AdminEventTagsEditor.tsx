...
export default function AdminEventTagsEditor() {
	...
	const editSection = (section: AdminEventTagSection) => openSheet(<AdminEventTagSectionSheet section={section} onSaved={setCatalogue} />);
	...
	return (
		<main className={styles.page}>
			...
			<div className={styles.adminToolbar}>
				...
				>
					<SymbolIcon name={isReordering ? "checkmark" : "arrow.up.arrow.down"} />
							...
							<div key={tag.id} className={styles.rowWithAction}>
								...
								{isReordering ? (
									<div className={styles.reorderButtons}>
										<button type="button" onClick={() => void moveTag(tag.id, -1)} aria-label={`Move ${tag.displayName} up`}>
											<span aria-hidden="true">↑</span>
										</button>
										<button type="button" onClick={() => void moveTag(tag.id, 1)} aria-label={`Move ${tag.displayName} down`}>
											<span aria-hidden="true">↓</span>
										</button>
									</div>
								) : null}
