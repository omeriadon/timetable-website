...
export default function AdminAppVersionEditor() {
	...
	const valid = useMemo(() => {
		...
		return version(draft.appVersion) && version(draft.macVersion) && draft.appBuild >= 0 && draft.macBuild >= 0;
	}, [draft]);

	const update = <K extends keyof AppVersionRequirement>(key: K, value: AppVersionRequirement[K]) => {
	...
	const save = async () => {
		...
		setStatus(null);
		try {
			const saved = await apiRequest<AppVersionRequirement>("v1/administration/app-version", {
				method: "PUT",
