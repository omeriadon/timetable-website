...
export default function WeekView({ subjects, friends }: { subjects: TimetableSubject[]; friends: DashboardData["friends"] }) {
	...
	return (
		<section className={styles.week} aria-label="Weekly timetable">
			<div className={styles.weekHeader}>{TIMETABLE_DAYS.map((day) => <span key={day}>{day}</span>)}</div>
				...
				{TIMETABLE_SESSIONS.map((session) => (
					<div key={session.value} className={styles.weekRow}>
						...
						{TIMETABLE_DAYS.map((day, dayIndex) => {
							...
							const isSelected = selectedSlot?.day === dayIndex && selectedSlot.session === session.value;
							return (
								<button key={day} type="button" className={styles.lessonButton} aria-label={`Open ${subject.id} on ${day}`} onClick={() => {
									...
									<article className={isSelected ? `${styles.lesson} ${styles.lessonSelected}` : styles.lesson} style={{ background: colour(subject) }}><span>{subject.symbol}</span><strong>{subject.id}</strong></article>
								</button>
							);
						})}
					</div>
			...
			{selectedSlot && selectedSubject ? <section className={styles.selectedLesson}><div><span className={styles.selectedEyebrow}>YOU</span><strong>{selectedSubject.symbol} {selectedSubject.id}</strong></div><span>{TIMETABLE_DAYS[selectedSlot.day]} · period {periodLabel(selectedSlot.session)}</span></section> : null}
