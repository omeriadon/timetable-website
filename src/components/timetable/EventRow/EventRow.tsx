...
export default function EventRow({ event, prominent = false }: { event: CalendarEvent; prominent?: boolean }) {
	const { openSheet } = useSheet();
	return (
		<button
			type="button"
			className={prominent ? styles.plannerEvent : styles.eventRow}
			onClick={() => openSheet(<CalendarEventSheet event={event} onChanged={() => undefined} />)}
		>
			<span className={styles.eventSymbol} aria-hidden="true">{event.symbol}</span>
			<div>
