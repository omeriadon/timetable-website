...
export default function WeekTimetable({ subjects }: { subjects: TimetableSubject[] }) {
	return (
				...
				{TIMETABLE_SESSIONS.map((session) => (
					<div key={session.value} className={styles.weekRow}>
						...
						{TIMETABLE_DAYS.map((day, dayIndex) => {
							...
							return subject ? <article key={day} className={styles.lesson} style={{ background: `rgb(${Math.round(subject.colour.r * 255)} ${Math.round(subject.colour.g * 255)} ${Math.round(subject.colour.b * 255)})` }}><span>{subject.symbol}</span><strong>{subject.id}</strong></article> : <div key={day} />;
						})}
					</div>
				))}
			</div>
