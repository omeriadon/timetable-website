...
export default function TodayView({ events, subjects, grades, schoolCalendar, schoolWeather }: {
	...
	return (
		<>
			<header className={styles.todayHeader}>
				<p>{schoolWeather ? `${Math.round(schoolWeather.temperatureCelsius)}°C　${weatherLabel(schoolWeather.conditionCode)}　${Math.round(schoolWeather.precipitationChance * 100)}%　UV ${schoolWeather.uvIndex}` : "Weather unavailable"}</p>
				<h1>{new Intl.DateTimeFormat("en-AU", { weekday: "long", day: "numeric", month: "long" }).format(today)}</h1>
			...
			{noSchool ? <section className={styles.paperCard}><h2>No School Today</h2><p>{noSchool.label}</p></section> : null}
			<section className={styles.timelineCard} aria-label="Today's school timeline">
				<div className={styles.timelineHeader}><h2>School Day</h2><span>{dayIndex >= 0 && dayIndex < 5 ? "8:50 am – 3:30 pm" : "No classes today"}</span></div>
				{dayIndex >= 0 && dayIndex < 5 ? (
					<div className={styles.timelineList}>
						{schoolPeriods.map((period) => {
							const subject = subjects.find((item) => item.slots.some((slot) => slot.day === dayIndex && slot.session === period.session));
							const active = isCurrentPeriod(period.start, period.end);
							return (
								<article key={period.label} className={active ? styles.timelineRowActive : styles.timelineRow}>
...
function todayDayIndex() { return new Date().getDay() - 1; }
...
function weatherLabel(conditionCode: string) {
	return conditionCode.replace(/([a-z])([A-Z])/g, "$1 $2").replace(/^./, (character) => character.toUpperCase());
}
...
function termWeekLabel(calendar: DashboardData["schoolCalendar"]) {
	...
	if (!term) return null;
	const monday = new Date(term.start.year, term.start.month - 1, term.start.day);
