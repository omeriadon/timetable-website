		...
		<section className={styles.planner}>
			...
			{schoolCalendar.termRanges.map((term) => (
				<button
					...
					onClick={() => openSheet(<TermDateSheet label={term.label} start={term.start} end={term.end} />)}
					aria-label={`Open ${term.label} dates`}
				>
					<span className={styles.eventSymbol} aria-hidden="true">▣</span>
					<div><strong>{term.label}</strong><span>{formatDateRange(term.start, term.end)}</span></div>
