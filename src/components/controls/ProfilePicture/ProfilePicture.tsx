...
export default function ProfilePicture({
	...
	const foreground = appearance?.foregroundColour ?? {
		r: 1,
		g: 1,
		b: 1,
		a: 1,
...
function proxiedPhotoURL(url: string) {
	try {
		const parsed = new URL(url, "https://timetable.invalid");
		const apiPathIndex = parsed.pathname.indexOf("/api/");
		if (apiPathIndex === -1) {
			return url;
		}

		const path = parsed.pathname.slice(apiPathIndex + "/api/".length);
		return `/web-api/${path}${parsed.search}`;
	} catch {
		return url;
	}
}
