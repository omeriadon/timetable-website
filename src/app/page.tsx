...
export default function Home() {
	...
	useEffect(() => {
		...
		if (requestedMode === "today" || requestedMode === "week" || requestedMode === "planner") {
	...
	const events = useMemo(() => {
		...
		return [...data.events.globalEvents, ...data.events.privateEvents]
			.sort((left, right) => `${left.date.year}${left.date.month}${left.date.day}`.localeCompare(`${right.date.year}${right.date.month}${right.date.day}`))
			.slice(0, 5);
	}, [data]);
