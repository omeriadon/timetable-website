...
async function handler({ request, params }: { request: Request; params: { _splat?: string } }) {
	...
	if (path === "session" && request.method === "GET") {
		const { response: upstream, tokens } = await authenticatedPMSTTRequest("v1/account");
	...
	if (path.startsWith("auth/") && request.method === "POST") {
		...
		if (!["request-code", "verify-code-register", "login"].includes(action)) return Response.json({ error: { reason: "Unknown authentication action." } }, { status: 404 });
		const body = await request.json();
		const upstream = await pmsttRequest(`v1/auth/${action}`, { method: "POST", body: JSON.stringify({ ...body, platform: "website" }) });
		const payload = await upstream.json().catch(() => ({})) as TokenResponse;
		const response = Response.json(upstream.ok && ["verify-code-register", "login"].includes(action) ? { user: payload.user } : payload, { status: upstream.status, headers: { "Cache-Control": "no-store" } });
...
export const Route = createFileRoute("/web-api/$")({ server: { handlers: { GET: handler, POST: handler, PUT: handler, DELETE: handler } } });
